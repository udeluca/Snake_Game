...
function Board:generateBoard()
    ...
    -- Spawning player at the middle of the board
    self.player.gridHeadX = math.floor((BOARD_DIMENSIONS/2) - 1)
    self.player.gridHeadY = math.floor((BOARD_DIMENSIONS/2) - 1)
...
function Board:render()
    ...
    for col = 0, BOARD_DIMENSIONS - 1 do
        -- Drawing each board wall in the right spot (row and column)
        for row = 0, BOARD_DIMENSIONS - 1 do
            local walls = {}
            if row == 0 then
               table.insert(walls, { quad = gFrames.tiles[TILE_GRASS_LEFT_WALL],  dx = -1, dy = 0 })
            elseif row + 1 == BOARD_DIMENSIONS then
                table.insert(walls, { quad = gFrames.tiles[TILE_GRASS_RIGHT_WALL], dx =  1, dy = 0 })
            end
            if col == 0 then
                table.insert(walls, { quad = gFrames.tiles[TILE_GRASS_TOP_WALL],   dx = 0,  dy = -1 })
            elseif col + 1 == BOARD_DIMENSIONS then
                table.insert(walls, { quad = gFrames.tiles[TILE_GRASS_BOTTOM_WALL],dx = 0,  dy =  1 })
            end

            for k = 1, #walls do
                local wall = walls[k]
                love.graphics.draw(gTextures["tilemap"], wall["quad"], (row*TILE_SIZE + self.boardPosX) + wall["dx"]*TILE_SIZE, (col*TILE_SIZE + self.boardPosY) + wall["dy"]*TILE_SIZE, 0, SCALE_X, SCALE_Y)
            end
